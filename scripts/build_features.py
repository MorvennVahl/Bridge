...
mols = {
    ...
    for m in json.loads(Path(f"{H}/chembl_molecule_phase.json").read_text())
    + json.loads(Path(f"{H}/chembl_molecule_therap.json").read_text())
}
...
trecs = json.loads(Path(f"{H}/targets_chembl.json").read_text())
comps = {c["component_id"]: c for c in json.loads(Path(f"{H}/target_components.json").read_text())}
pcs = {
    p["protein_class_id"]: p
    for p in json.loads(Path(f"{H}/protein_classification.json").read_text())
}
sym2ensg = json.loads(Path(f"{H}/targets_sym2ensg.json").read_text())
ot = json.loads(Path(f"{H}/targets_opentargets.json").read_text())
kegg_match = pd.read_csv(f"{H}/kegg_name_match.csv")
kegg_entries = json.loads(Path(f"{H}/kegg_entries.json").read_text())
...
for f in [
    "provenance_chembl",
    "provenance_targets",
    "provenance_protein_class",
    "provenance_kegg",
    "provenance_openfda",
]:
    p = f"{H}/{f}.json"
    if os.path.exists(p):
        prov["sources"][f.replace("provenance_", "")] = json.loads(Path(p).read_text())
...
def ot_feats(g):
    ...
    if not t:
        return {}
    d = {
        "ot_approved_symbol": t.get("approvedSymbol"),
        "ot_biotype": t.get("biotype"),
        "ot_is_essential": t.get("isEssential"),
    }
    for x in t.get("tractability") or []:
        d[f"tract_{x['modality']}_{re.sub('[^a-z0-9]+', '_', x['label'].lower())}"] = bool(
            x["value"]
        )
    ...
    d["reactome_top_level_terms"] = "|".join(
        sorted({p["topLevelTerm"] for p in pw if p.get("topLevelTerm")})
    )
    return d


tgt = pd.concat(
    [tgt, pd.DataFrame([ot_feats(g) for g in tgt.ensembl_gene_id], index=tgt.index)], axis=1
)
...
untarg = (
    mech.groupby("omop_concept_id")
    .agg(
        n_mechanism_rows_any=("mechanism_of_action", "size"),
        n_mechanism_rows_without_target=("target_chembl_id", lambda s: int(s.isna().sum())),
    )
    .reset_index()
)
...
kf = pd.DataFrame(
    [kegg_feats(d) if isinstance(d, str) else {} for d in kegg_match.kegg_drug_id],
    index=kegg_match.index,
)
kegg_blk = pd.concat(
    [kegg_match[["ingredient_name", "kegg_drug_id", "kegg_n_candidates"]], kf], axis=1
)
...
for _, r in ing.iterrows():
    ...
    def g(k: str, m: dict = m, p: dict = p) -> object:
        return p.get(k) if (m.get(k) in (None, "")) and p else m.get(k)
    ...
    d["atc_codes"] = "|".join(atc)
    d["n_atc_codes"] = len(atc)
    d["atc_l1"] = "|".join(sorted({a[0] for a in atc}))
    d["n_atc_l1_groups"] = len({a[0] for a in atc})
...
molblk["ro5_like"] = (
    (molblk.full_mwt <= 500) & (molblk.alogp <= 5) & (molblk.hbd <= 5) & (molblk.hba <= 10)
)
molblk["n_routes_labelled"] = molblk[["oral", "parenteral", "topical"]].sum(axis=1, min_count=1)

# ------------------------------------------------------------ warnings block --
wag = (
    warn.groupby("omop_concept_id")
    .agg(
        ...
        warning_year_min=("warning_year", "min"),
    )
    .reset_index()
    .rename(columns={"omop_concept_id": "_jk"})
)
wag["_jk"] = wag._jk.astype(float)

mtag = (
    metab.groupby("omop_concept_id")
    .agg(
        ...
        chembl_metabolite_names=(
            "metabolite_name",
            lambda s: "|".join(sorted(set(s.dropna()))[:12]),
        ),
    )
    .reset_index()
    .rename(columns={"omop_concept_id": "_jk"})
...
gf = (
    tgt.dropna(subset=["ensembl_gene_id"])
...
for cid, d in long.groupby("omop_concept_id"):
    ...
    r = {
        "omop_concept_id": cid,
        "n_mechanism_rows": len(d),
        "n_targets_chembl": d.target_chembl_id.nunique(),
        ...
        "dominant_target_class": (
            sub.chembl_protein_class_L1.mode().iat[0]
            if len(sub) and sub.chembl_protein_class_L1.notna().any()
            else None
        ),
    }
    for c in L1:
        ...
        r.update(
            {
                ...
                "target_safety_events": "|".join(
                    sorted({e for s in sub.safety_events.dropna() for e in s.split("|") if e})
                )[:500],
                "constraint_lof_upperBin_min": sub.get(
                    "constraint_lof_upperBin", pd.Series(dtype=float)
                ).min(),
                "constraint_lof_upperBin_mean": sub.get(
                    "constraint_lof_upperBin", pd.Series(dtype=float)
                ).mean(),
                "constraint_mis_upperBin_mean": sub.get(
                    "constraint_mis_upperBin", pd.Series(dtype=float)
                ).mean(),
                ...
                "reactome_top_level_terms": "|".join(
                    sorted(
                        {
                            t
                            for s in sub.reactome_top_level_terms.dropna()
                            for t in s.split("|")
                            if t
                        }
                    )
...
for nm in ["label_any", "label_boxed_warning", "label_pregnancy", "label_drug_interactions"]:
    ...
    if os.path.exists(p):
        ofda[nm] = {d["term"].lower(): d["count"] for d in json.loads(Path(p).read_text())}
...
feat = (
    ing[
        [
            ...
            "chembl_parent_id",
            "chembl_match_method",
            "chembl_match_n_candidates",
            "chembl_fuzzy_candidate_id",
            "chembl_fuzzy_candidate_name",
            "chembl_fuzzy_ratio",
        ]
    ]
...
for c in [
    "n_drug_warnings",
    "chembl_n_metabolism_rows",
    "n_mechanism_rows",
    "n_targets_chembl",
    "n_target_genes",
] + [c for c in feat.columns if c.startswith("n_genes_")]:
    if c in feat:
...
lab_adj = ing[["omop_concept_id", "ingredient_name"]].merge(
    degree, on="omop_concept_id", how="left"
)
...
SRC = {
    "OMOP": (
        "OMOP/RxNorm via CEM",
        "cem_ingredients.csv (local Postgres cem, cem_output.cem_unified)",
        "",
    ),
    ...
    "CHEMBL_WARN": (
        "ChEMBL",
        f"{CH_REL} ({CH_DATE})",
        "https://www.ebi.ac.uk/chembl/api/data/drug_warning.json",
    ),
    "CHEMBL_METAB": (
        "ChEMBL",
        f"{CH_REL} ({CH_DATE})",
        "https://www.ebi.ac.uk/chembl/api/data/metabolism.json",
    ),
    "CHEMBL_TGT": (
        "ChEMBL",
        f"{CH_REL} ({CH_DATE})",
    ...
    "CHEMBL_PC": (
        "ChEMBL",
        f"{CH_REL} ({CH_DATE})",
        "https://www.ebi.ac.uk/chembl/api/data/target_component.json + protein_classification.json",
    ),
    "UNIPROT": (
        "UniProtKB",
        "current release at retrieval",
    ...
    "KEGG": (
        "KEGG DRUG",
        "current release at retrieval",
        "https://rest.kegg.jp/list/drug + /get/dr:<id>",
    ),
    "OPENFDA": (
        "openFDA",
        "see provenance.json last_updated",
        "https://api.fda.gov/drug/label.json (count aggregation)",
    ),
    "DERIVED": ("Derived in build_features.py", "-", "-"),
    "CEM": (
        "OHDSI Common Evidence Model",
        "local Postgres cem",
...
DESC = {
    ...
    "roster_name": (
        "identity",
        "Name as given in the user's indication roster, where applicable",
        "ROSTER",
    ),
    "indication": (
        "identity",
        "Indication block the drug was listed under in the user's roster",
        "ROSTER",
    ),
    "source_class": ("identity", "Pharmacologic class as given in the user's roster CSV", "ROSTER"),
    "in_indication_roster": (
        "identity",
        "Present in the user's 97-exposure indication roster",
        "DERIVED",
    ),
    ...
    "chembl_id": (
        "identity",
        "ChEMBL molecule matched by exact normalized name or qualifier-stripped name",
        "CHEMBL_MOL",
    ),
    "chembl_parent_id": (
        "identity",
        "Parent molecule from ChEMBL molecule_hierarchy (salts resolved to parent)",
        "CHEMBL_MOL",
    ),
    "chembl_match_method": (
        "identity",
        "How the name was resolved; no fuzzy matches are used here",
        "DERIVED",
    ),
    "chembl_match_n_candidates": (
        "identity",
        "Number of distinct ChEMBL molecules sharing the matched name key",
        "DERIVED",
    ),
    "chembl_fuzzy_candidate_id": (
        "identity",
        "REVIEW ONLY. Nearest fuzzy name match; NOT used by any feature (high false-positive rate)",
        "DERIVED",
    ),
    "chembl_fuzzy_candidate_name": (
        "identity",
        "REVIEW ONLY. Preferred name of the fuzzy candidate",
        "DERIVED",
    ),
    ...
    "kegg_n_candidates": (
        "identity",
        "Number of KEGG entries sharing the matched name key",
        "DERIVED",
    ),
    "max_phase": (
        "development",
        "Highest development phase reached (4 = approved; -1 = unknown in ChEMBL)",
        "CHEMBL_MOL",
    ),
    "first_approval": ("development", "Year of first regulatory approval", "CHEMBL_MOL"),
    "therapeutic_flag": (
        "development",
        "Flagged by ChEMBL as a therapeutic (vs diagnostic/reagent)",
        "CHEMBL_MOL",
    ),
    "availability_type": ("development", "Raw ChEMBL availability code", "CHEMBL_MOL"),
    "availability_label": (
        "development",
        "Availability decoded: prescription_only / otc / discontinued / withdrawn / unknown",
        "DERIVED",
    ),
    "dosed_ingredient": (
        "development",
        "Appears as a dosed ingredient in a marketed product",
        "CHEMBL_MOL",
    ),
    "withdrawn_flag": (
        "safety",
        "Withdrawn from market in at least one jurisdiction",
        "CHEMBL_MOL",
    ),
    "black_box_warning": ("safety", "ChEMBL black-box warning flag on the molecule", "CHEMBL_MOL"),
    "n_drug_warnings": (
        "safety",
        "Count of ChEMBL drug_warning records (boxed warnings / withdrawals)",
        "CHEMBL_WARN",
    ),
    "warning_types": ("safety", "Warning types, pipe-delimited", "CHEMBL_WARN"),
    "warning_classes": (
        "safety",
        "Toxicity classes (cardiotoxicity, teratogenicity, ...), pipe-delimited",
        "CHEMBL_WARN",
    ),
    ...
    "molecule_type": (
        "chemistry",
        "Small molecule / Protein / Oligosaccharide / ...",
        "CHEMBL_MOL",
    ),
    ...
    "n_routes_labelled": (
        "exposure",
        "Count of oral/parenteral/topical route flags set",
        "DERIVED",
    ),
    "indication_class": ("pharmacology", "ChEMBL free-text indication class", "CHEMBL_MOL"),
    "usan_stem_definition": (
        "pharmacology",
        "USAN stem definition (encodes mechanism class from the name)",
        "CHEMBL_MOL",
    ),
    ...
    "n_atc_l1_groups": (
        "pharmacology",
        "Number of distinct ATC level-1 groups (breadth of clinical use)",
        "DERIVED",
    ),
    "atc_l3": ("pharmacology", "ATC level-4 code prefixes (pharmacological subgroups)", "DERIVED"),
    "ro5_like": (
        "chemistry",
        "Derived Lipinski rule-of-five compliance (MW<=500, alogp<=5, HBD<=5, HBA<=10)",
        "DERIVED",
    ),
    "n_mechanism_rows": (
        "mechanism",
        "ChEMBL mechanism rows with an assigned target",
        "CHEMBL_MECH",
    ),
    "n_mechanism_rows_any": (
        "mechanism",
        "ChEMBL mechanism rows including those with no target assigned",
        "CHEMBL_MECH",
    ),
    "n_mechanism_rows_without_target": (
        "mechanism",
        "Mechanism rows where ChEMBL records no target",
        "CHEMBL_MECH",
    ),
    "mechanism_recorded_but_target_unassigned": (
        "mechanism",
        "Mechanism is documented but no target is assigned (e.g. hydralazine)",
        "DERIVED",
    ),
    "n_targets_chembl": (
        "mechanism",
        "Distinct ChEMBL targets (may be complexes or families)",
        "CHEMBL_MECH",
    ),
    ...
    "action_types": (
        "mechanism",
        "Action types (INHIBITOR/AGONIST/BLOCKER/...), pipe-delimited",
        "CHEMBL_MECH",
    ),
    "mechanisms_of_action": (
        "mechanism",
        "ChEMBL mechanism-of-action strings, pipe-delimited (truncated at 500 chars)",
        "CHEMBL_MECH",
    ),
    "has_family_or_complex_target": (
        "mechanism",
        "At least one target is a protein family or complex, not a single protein",
        "DERIVED",
    ),
    "frac_direct_interaction": (
        "mechanism",
        "Fraction of mechanism rows flagged as direct target interaction",
        "CHEMBL_MECH",
    ),
    "dominant_target_class": (
        "mechanism",
        "Modal ChEMBL level-1 protein class across target genes",
        "DERIVED",
    ),
    "has_chembl_mechanism": (
        "mechanism",
        "At least one mechanism row with an assigned target",
        "DERIVED",
    ),
    "safety_liabilities_sum": (
        "target biology",
        "Sum of Open Targets curated safety liabilities across target genes",
        "OT",
    ),
    "safety_liabilities_max": (
        "target biology",
        "Max safety liabilities on any single target gene",
        "OT",
    ),
    "n_genes_with_safety_liability": (
        "target biology",
        "Count of target genes with >=1 curated safety liability",
        "OT",
    ),
    "target_safety_events": (
        "target biology",
        "Union of safety-liability event terms across target genes (truncated)",
        "OT",
    ),
    "constraint_lof_upperBin_min": (
        "target biology",
        "Most constrained LoF decile bin across target genes (lower = more constrained)",
        "OT",
    ),
    "constraint_lof_upperBin_mean": (
        "target biology",
        "Mean LoF constraint decile bin across target genes",
        "OT",
    ),
    "constraint_mis_upperBin_mean": (
        "target biology",
        "Mean missense constraint decile bin across target genes",
        "OT",
    ),
    "n_essential_genes": (
        "target biology",
        "Count of target genes flagged essential in DepMap via Open Targets",
        "OT",
    ),
    "n_reactome_pathways_union": (
        "target biology",
        "Summed Reactome pathway memberships across target genes",
        "OT",
    ),
    "reactome_top_level_terms": (
        "target biology",
        "Union of Reactome top-level pathway terms for target genes (truncated)",
        "OT",
    ),
    "chembl_n_metabolism_rows": (
        "metabolism",
        "ChEMBL metabolism records for the drug or its parent",
        "CHEMBL_METAB",
    ),
    "chembl_metab_enzymes": (
        "metabolism",
        "Enzymes named in ChEMBL metabolism records, pipe-delimited",
        "CHEMBL_METAB",
    ),
    "chembl_metabolite_names": (
        "metabolism",
        "Named metabolites (first 12), pipe-delimited",
        "CHEMBL_METAB",
    ),
    "kegg_metab_enzymes": (
        "metabolism",
        "Metabolising enzymes from the KEGG DRUG METABOLISM field",
        "KEGG",
    ),
    "kegg_transporters": ("metabolism", "Transporters from the KEGG DRUG METABOLISM field", "KEGG"),
    "kegg_interaction_genes": (
        "interactions",
        "Genes named in the KEGG DRUG INTERACTION field (DDI partners)",
        "KEGG",
    ),
    ...
    "kegg_cyp_inhibition": (
        "interactions",
        "KEGG INTERACTION field records CYP inhibition",
        "KEGG",
    ),
    "kegg_cyp_induction": (
        "interactions",
        "KEGG INTERACTION field records CYP or enzyme induction",
        "KEGG",
    ),
    "kegg_transporter_inhibition": (
        "interactions",
        "KEGG INTERACTION field records transporter inhibition",
        "KEGG",
    ),
    "kegg_efficacy": ("pharmacology", "KEGG EFFICACY string (truncated at 300 chars)", "KEGG"),
    "openfda_has_boxed_warning_label": (
        "safety",
        "Ingredient appears among generic names with a boxed_warning SPL section",
        "OPENFDA",
    ),
}
PATTERNS = [
    (
        re.compile(r"^n_genes_tract_"),
        (
            "target biology",
            "Count of target genes with this Open Targets tractability bucket true",
            "DERIVED",
        ),
    ),
    (
        re.compile(r"^n_genes_"),
        ("mechanism", "Count of target genes in this ChEMBL level-1 protein class", "DERIVED"),
    ),
    (
        re.compile(r"^kegg_substrate_CYP"),
        ("metabolism", "KEGG METABOLISM names this CYP as a metabolising enzyme", "KEGG"),
    ),
    (
        re.compile(r"^openfda_label_.*_n_labels$"),
        (
            "safety",
            "Number of SPL label documents for this generic name with the section present (top-1000 aggregation only)",
            "OPENFDA",
        ),
    ),
    (re.compile(r"^tract_"), ("target biology", "Open Targets tractability bucket", "OT")),
    (
        re.compile(r"^constraint_"),
        ("target biology", "Open Targets gnomAD genetic-constraint statistic", "OT"),
    ),
    (re.compile(r"^ot_"), ("target biology", "Open Targets target attribute", "OT")),
    (
        re.compile(r"^uniprot_|^protein_name$|^gene_symbol$|^aa_length$|^accession$"),
        ("target biology", "UniProtKB annotation", "UNIPROT"),
    ),
    (
        re.compile(r"^ensembl_gene_id$"),
        ("target biology", "Ensembl gene id resolved from HGNC symbol", "ENSEMBL"),
    ),
    (
        re.compile(r"^target_|^component_|^chembl_protein_class"),
        ("target biology", "ChEMBL target / component definition", "CHEMBL_TGT"),
    ),
    (
        re.compile(
            r"^n_safety_liabilities$|^safety_events$|^safety_datasources$|^n_reactome_pathways$"
        ),
        ("target biology", "Open Targets target attribute", "OT"),
    ),
    (
        re.compile(r"^cem_"),
        (
            "LABEL-ADJACENT",
            "Derived from the CEM association file itself (same FAERS lineage as the labels) - nuisance/degree term, not a biology predictor",
            "CEM",
        ),
    ),
    (
        re.compile("|".join(f"^{re.escape(p)}$" for p in PROP)),
        ("chemistry", "ChEMBL computed physicochemical property", "CHEMBL_MOL"),
    ),
    (
        re.compile(
            r"^(action_type|mechanism_of_action|direct_interaction|molecular_mechanism|disease_efficacy|mechanism_source)$"
        ),
        ("mechanism", "ChEMBL mechanism record field", "CHEMBL_MECH"),
    ),
]
rows = []
for table, df in [
    ("ingredient_features.csv", feat),
    ("target_features.csv", tgt),
        ...
        rows.append(
            {
                ...
                "non_null_fraction": round(float(nn), 4),
            }
        )
...
print(
    "with chembl id",
    feat.chembl_id.notna().sum(),
    "| with mechanism",
    feat.has_chembl_mechanism.sum(),
    "| with kegg",
    feat.kegg_drug_id.notna().sum(),
)
