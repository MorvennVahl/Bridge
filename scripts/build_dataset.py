...
DRUG_FILES = ["ingredient_features.csv", "target_features.csv",
              "ingredient_target_long.csv", "ingredient_features_label_adjacent.csv"]
...
def build_labels(cem: pd.DataFrame) -> pd.DataFrame:
    ...
    df["y_faers_signal"] = (df.faers_prr.ge(2)
                            & df.faers_chi_square.ge(4)
                            & df.faers_case_count.ge(3)).fillna(False)
    ...
    keep = ["ingredient_concept_id", "condition_concept_id",
            "in_faers", "in_semmeddb", "in_eu_label",
            "faers_case_count", "faers_prr", "faers_chi_square",
            "semmeddb_harm_sentences", "semmeddb_benefit_sentences",
            "semmeddb_negated_sentences",
            "y_faers_signal", "y_semmeddb_causes", "y_semmeddb_treats", "y_any_harm"]
    return df[keep]
...
def primary_gene_groups(itl: pd.DataFrame, ingredients: list[int]) -> pd.Series:
    ...
    prim = (tgt.sort_values(["omop_concept_id", "disease_efficacy", "direct_interaction"],
                            ascending=[True, False, False])
               .groupby("omop_concept_id").gene_symbol.first())
    return pd.Series(
...
def target_component_groups(itl: pd.DataFrame, ingredients: list[int]) -> pd.Series:
    ...
    g = nx.Graph()
    for row in itl[itl.gene_symbol.notna()][["omop_concept_id", "gene_symbol"]].drop_duplicates().itertuples(index=False):
...
def assign_folds(group_sizes: pd.Series, seed: int) -> dict[str, str]:
    ...
    current = {f: 0.0 for f in PROPORTIONS}
...
def main() -> int:
    ...
    ap.add_argument("--scheme", choices=["primary_gene", "target_component"],
                    default="primary_gene")
    ...
    cem = pd.read_csv(DATA / "cem_ingredient_condition_associations.csv",
                      dtype={"ingredient_concept_id": "int64",
    ...
    groups = (primary_gene_groups(itl, ingredients) if args.scheme == "primary_gene"
    ...
    labelled = labels.merge(assignment[["ingredient_concept_id", "fold", "group_key"]],
                            on="ingredient_concept_id", how="left")
    ...
    (split_dir / "split_provenance.json").write_text(json.dumps({
        "scheme": args.scheme,
        "seed": args.seed,
        "proportions": PROPORTIONS,
        "grouping_unit": ("primary target gene from ingredient_target_long.csv; "
