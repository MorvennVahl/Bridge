...
MOL_FIELDS = ("molecule_chembl_id,pref_name,molecule_synonyms,max_phase,first_approval,"
...
def bulk(name, ep, params, key, limit=1000):
    ...
    with open(f"handoff/chembl_{name}.json", "w") as fh:
        json.dump(out, fh)
    return {"table": name, "endpoint": f"{B}/{ep}.json", "query_params": params,
            "n_rows": len(out), "total_count_reported": total,
...
if __name__ == "__main__":
    ...
    json.dump(prov, open("handoff/provenance_chembl.json", "w"), indent=2)
