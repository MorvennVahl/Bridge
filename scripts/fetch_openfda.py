...
QUERIES = {
    ...
    "label_any": ("/drug/label.json", {"count": "openfda.generic_name.exact"}),
    "label_boxed_warning": (
        "/drug/label.json",
...
prov = {
    ...
    "queries": {},
    "note": (
        "openFDA count aggregations return the top 1,000 terms only; ingredients outside "
...
for name, (path, params) in QUERIES.items():
    ...
    j = r.json()
    rows = j.get("results", [])
    Path(f"handoff/openfda_{name}.json").write_text(json.dumps(rows))
    prov["queries"][name] = {
        "endpoint": BASE + path,
        "params": p,
        "n_terms": len(rows),
...
Path("handoff/provenance_openfda.json").write_text(json.dumps(prov, indent=2))
